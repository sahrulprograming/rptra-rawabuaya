...
class Ajax extends CI_Controller
{
    ...
    public function data_absensi($ID_rptra = null)
    {
        $output = [];
        $data = '';
        $modal = '';
        ...
        $this->db->where('u.ID_rptra', $ID_rptra);
                ...
                if ($date > date('d-F-Y') || $date == date('d-F-Y') && date('H:i') < '06:30') {
                    ...
                    if ($absensi) {
                        if ($absensi->status == 'hadir') {
                            ...
                            $keterangan = 'Anda masuk pada jam <br>' . format_jam($absensi->created_at);
                            if ($absensi->jam_pulang) {
                                $keterangan = 'Anda bekerja selama <br>' . jumlah_jam_masuk(format_jam($absensi->created_at), format_jam($absensi->jam_pulang));
                            }
                            $foto = $absensi->foto;
                        } else {
                            ...
                            $("#hapus-absen-' . $ID_user . '").on("click", function() {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        window.location.replace("' . base_url('admin/hapus/absensi') . "/$ID_user/" . $date . "/$foto" . '")
                                    }
                                })
                            })
                            </script>
